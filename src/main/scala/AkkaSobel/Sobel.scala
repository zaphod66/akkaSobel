...
object Sobel extends App {
  ...
  class Master(val config: Configuration) extends Actor {
    ...
    val medianRouter   = context.actorOf(Props(new MedianOp(tmp1Image, 2)).withRouter(RoundRobinRouter(noOfWorkers)), name = "medianRouter")
    ...
    var ops = List(medianRouter, sobelRouter, thresRouter, invertRouter, dilate1Router)
